...
cabeceraTeclado() {
    ...
    if [[ "$conEntradaCPU" == 's' || "$conEntradaCPU" == "1" ]]; then
...
CabeceraAleatorio() {
    ...
    if [[ "$conEntradaCPU" == "1" || "$conEntradaCPU" == 's' ]]; then
...
entradaTeclado() {
    ...
    until [[ $tamMem =~ [0-9] && $tamMem -gt 0 ]]; do
    ...
    until [[ "$conEntradaCPU" == 's' || "$conEntradaCPU" == 'n' ]]; do
        echo -n -e " ¿Quiere que haya \e[1;33mtiempos de entrada a CPU\e[0m?(s/n): "
        read conEntradaCPU
    done
    sleep 0.2
    ...
    if [[ "$conEntradaCPU" == 's' ]]; then
        ...
        until [[ $tiempoPorPagina =~ [0-9] && $tiempoPorPagina -gt 0 ]]; do
    ...
    until [[ "$prioridadMayorMMenorN" == "m" || "$prioridadMayorMMenorN" == "n" ]]; do
    ...
    until [[ $factReub =~ [0-9] && $factReub -ge 0 ]]; do
    ...
    while [[ $otroProc == "s" ]]; do
        ...
        until [[ ${paginasCodigo[$p]} =~ [0-9] && ${paginasCodigo[$p]} -gt 0 && ${paginasCodigo[$p]} -lt ${nMarcos[$p]} ]]; do
        ...
        while [[ prProcesos[$p] -gt $maximoPrioridad || prProcesos[$p] -lt $minimoPrioridad ]]; do
        ...
        while [[ $otraMemoria == "s" ]]; do
            ...
            until [[ ${direcciones[$p,$pag]} =~ ^[0-9]+$ && ${direcciones[$p,$pag]} -gt 0 ]]; do
                echo "ESTO AQUI"
            ...
            # echo "tamaño de pagina: $tamPag // lo escrito ${direcciones[$p,$pag]}"
            # echo "${direcciones[$p,$pag]}/${tamPag} EL RESULTADO QUE NOS DEBERIA DAR"
            ...
            until [[ $otraMemoria == "s" || $otraMemoria == "n" ]]; do
        ...
        if [[ $((${nMarcos[$p]} - ${paginasCodigo[$p]})) -gt ${tEjec[$p]} ]]; then
            bool=1
        else
            bool=0
        fi
...
imprimefiles() {
    ...
    if [[ $# -gt 0 ]]; then #Si se han pasado argumentos.
        for ((i = 1; i <= $max; i++)); do
...
IntroduccionDeDatosPorFichero() {
    ...
    # los procesos
    for ((fila = $nParametrosGenerales + 1; fila <= $maxFilas; fila++)); do
        ...
        until [[ $(echo $lectura2 | cut -f $posic -d ";") == "" ]]; do #for (( n = 0; n < ${maxpags[$p]}; n++ ))
...
entradaAleatoria() {
    ...
    if [[ "$conEntradaCPU" == 's' ]]; then
        ...
        if [[ $minTiempoPorPagina -eq $maxTiempoPorPagina ]]; then
...
genProAleatorios() {
    ...
    for ((i = 0; i < $procesosAGenerar; i++)); do
        ...
        if [[ $tiempoPorPagina -ne 0 ]]; then
            if [[ $marcosProcesoTemp -le $minPaginasCodigo ]]; then
                minPaginasCodigo=0
            fi
            local pagsCodlimit=0
            if [[ $(($marcosProcesoTemp - 1)) -lt $maxPaginasCodigo ]]; then
...
imprimeProcesosAleatorio() {
    ...
    if [[ "$conEntradaCPU" == 's' || $tiempoPorPagina -ne 0 ]]; then
...
seleccionRR() {
    ...
    until [[ "$opcionEjec" == "1" || "$opcionEjec" == "2" || "$opcionEjec" == "3" || "$opcionEjec" == "4" || "$opcionEjec" == "5" ]]; do
...
diagramaresumen() {
    ...
    for tEspera in ${tiempoDeEspera[@]}; do
    ...
    for tRetorno in ${tiempoDeRetorno[@]}; do
...
imprimeOptimo() {
    ...
    for ((i = 0; i < $marcosMem; i++)); do
        ...
        if [[ "$procesoAct" == 0 || "$procesoAct" == '.' || "${VectorMemoriaPag[$i]}" == '+' ]]; then
            continue
        fi
        ...
        if [[ "${VectorMejorContadorProceso[$procesoAct]}" != '-' ]]; then

            if [[ "$contadorMarcoActual" == '-' ]]; then
                VectorMejorContadorProceso[$procesoAct]='-'
                VectorMarcoReemplazo[$procesoAct]=$i
            elif [[ "$contadorMarcoActual" == '*' && "$mejorContadorProceso" != '*' ]]; then
                VectorMejorContadorProceso[$procesoAct]='*'
                VectorMarcoReemplazo[$procesoAct]=$i
            elif [[ "$mejorContadorProceso" == '*' ]]; then
                continue
            elif [[ $mejorContadorProceso -lt $contadorMarcoActual ]]; then
    ...
    for i in ${VectorMarcoReemplazo[@]}; do
    ...
    for ((linea = 0; ; linea++)); do
        ...
        for ((m = primerMarco; m <= ultimoMarco; m++)); do
            ...
            if [[ ${VectorMemoriaApuntador[$m]} -eq 1 ]]; then
                printf -v tempBloque "\e[3${colorines[$temp]};1m%$((anchoBloque - 1))s\e[0m" ""
                echo -en "${tempBloque// /^}"
                echo -en " "
...
EjecutaRoundRobin() {
    ...
    while [[ $(CompruebaFinDeEjecucion) -eq 0 ]]; do
...
RealizarReubicacion() {
    ...
    for proceso in ${ordenados[@]}; do
        ...
        if [[ "$estadoProc" != "Fuera de sistema" && "$estadoProc" != "Finalizado" && "$estadoProc" != "En espera" ]]; then
    ...
    for proceso in ${procesosEnMemoria[@]}; do
        for ((i = 0; i < ${indiceMarcosProceso[$proceso]}; i++)); do
    ...
    for ((i = $contador; i < $marcosMem; i++)); do
        VectorMemoriaProceso[$i]='.'
        VectorMemoriaPag[$i]='-'
        VectorMemoriaContadorOptimo[$i]='-'
...
ComprobarReubicacion() {
    ...
    if [[ "$factReub" == 0 ]]; then
        ...
        for ((i = 0; i < $marcosMem; i++)); do
            ...
            if [[ "${VectorMemoriaProceso[$i]}" == '.' ]]; then
                marcosLibres=0

                for ((j = i; j <= $marcosMem; j++)); do
                    if [[ "${VectorMemoriaProceso[$j]}" != '.' ]]; then
        ...
        for ((i = 0; i < $marcosMem; i++)); do
            ...
            if [[ ${marcosVacios[$i]} -ge ${nMarcos[$1]} ]]; then
...
EjecutaProceso() {
    if [[ $hayEntradaCPU -eq 1 && $conEntradaCPU -eq 1 ]]; then
        EjecutaEntradasCPU
        return 0
    fi

    if [[ $conEntradaCPU -eq 1 ]]; then
        #Comprobamos que se está ejecutando un proceso
        if [[ $procesoEjecutandose -ne 0 ]]; then
...
HaFinalizadoProceso() {
    ...
    if [[ ${tiempoEjecutado[$1]} -eq ${tEjec[$1]} ]]; then
        echo 1
    else
        echo 0
    fi
...
GestionLlegadaProcesosAlSistema() {
    ...
    for ((i = $punteroLlegadaProcesos; i <= $nProc; i++)); do
        local proceso=${ordenados[$i]}

        if [[ $tiempoDelSistema -ge ${tLlegada[$proceso]} ]]; then
...
GestionLlegadaProcesosMemoria() {
    ...
    if [[ ${#colaEntradaMemoria[@]} -le 0 ]]; then
        return 0
    fi
    ...
    while [ $marcosRequeridos -le $espacioLibreMemoria ]; do
        ComprobarReubicacion $procesoAMeter

        if [[ $hayReubicacion -eq 1 ]]; then
            hayReubicacion=1
            RealizarReubicacion
        fi
        ...
        if [[ ${#colaEntradaMemoria[@]} -gt 1 ]]; then
            for ((i = 1; i < ${#colaEntradaMemoria[@]}; i++)); do
...
GestionPaginasProceso() {
    ...
    if [[ $(PaginaYaCargada $paginaPorIntroducir) -eq 0 ]]; then
        let nFallosPagina[$procesoEjecutandose]++
        #Si existen marcos libres en los asignados al proceso
        if [[ $(CalculaPaginasLibresProceso) -ge 1 ]]; then
            for ((i = 0; i < $marcosMem; i++)); do

                if [[ "${VectorMemoriaProceso[$i]}" == "$procesoEjecutandose" && "${VectorMemoriaPag[$i]}" == '-' ]]; then
...
GuardaHistorialPaginasProceso() {
    ...
    for ((i = 0; i < $marcosMem; i++)); do
        ...
        if [[ "${VectorMemoriaProceso[$i]}" != "$procesoEjecutandose" ]]; then
            continue
        fi

        if [[ "${VectorMemoriaPag[$i]}" == "-" ]]; then
            cadena="M$i"
            if [[ $apuntadorColocado -eq 0 ]]; then

                marcoApuntador=$marcoProceso
                apuntadorColocado=1
            fi
        elif [[ "${VectorMemoriaPag[$i]}" == '+' ]]; then
            cadena="M$i-+"
        else
            cadena="M$i-${VectorMemoriaPag[$i]}-${VectorMemoriaContadorOptimo[$i]}"

            if [[ $apuntadorColocado -eq 0 && "$maxContadorOptimo" != '*' ]]; then
                if [[ "${VectorMemoriaContadorOptimo[$i]}" == '*' ]]; then

                    maxContadorOptimo='*'
                    marcoApuntador=$marcoProceso

                elif [[ $maxContadorOptimo -lt ${VectorMemoriaContadorOptimo[$i]} ]]; then
...
ReemplazoDePaginaOptimo() {
    ...
    for ((i = 0; i < $marcosMem; i++)); do
        ...
        if [[ ${VectorMemoriaContadorOptimo[$i]} -gt $contadorOptimoMaximo ]]; then
            paginaAReemplazar=$i
...
CalculaContadorOptimo() {
    ...
    for ((i = 0; i < $marcosMem; i++)); do
        ...
        if [[ "$paginaDelMarco" == '+' ]]; then
            continue
        fi

        if [[ "${VectorMemoriaProceso[$i]}" == "$procesoACalcular" ]]; then
            ...
            if [[ "$paginaDelMarco" == '-' ]]; then
                ...
                for ((indexPaginas = $((${paginaActualProceso[$procesoACalcular]} + 1)); indexPaginas < ${maxpags[$procesoACalcular]}; indexPaginas++)); do
                    let contadorOptimo++

                    if [[ "$paginaDelMarco" == "${paginas[$procesoACalcular, $indexPaginas]}" ]]; then
                        flagExistePag=1
                        break
                    fi
...
PaginaYaCargada() {
    ...
    for ((i = 0; i < $marcosMem; i++)); do
        if [[ "${VectorMemoriaProceso[$i]}" == "$procesoEjecutandose" && "${VectorMemoriaPag[$i]}" == "$paginaAComprobar" ]]; then
            cargada=1
            break
        fi
    done
...
InsertarProcesoEnMemoria() {
    ...
    for ((i = $posicionEntradaMem; i < ${#VectorMemoriaProceso[@]}; i++)); do

        if [[ "${VectorMemoriaProceso[$i]}" == "." ]]; then

            # Rellenamos los marcos de código y datos
            if [[ $marcosDeCodigoYDatos -gt 0 ]]; then
...
SacarProcesoMemoria() {
    ...
    for ((i = 0; i < $marcosMem; i++)); do
        if [[ "$procesoASacar" == "${VectorMemoriaProceso[$i]}" ]]; then
            VectorMemoriaProceso[$i]="."
            VectorMemoriaPag[$i]="-"
            VectorMemoriaContadorOptimo[$i]="-"
            let marcosASacar--
...
ObtenerMarcosLibresMemoria() {
    ...
    for marco in "${VectorMemoriaProceso[@]}"; do
        if [[ "$marco" == "." ]]; then
            let marcosVacios++
        fi
...
ImprimeEstadoSistema() {
    ...
    if [[ $antiguoProcesoEjecutandose -gt 0 && $antiguoProcesoEjecutandose -ne $procesoEjecutandose ]]; then
    ...
    if [[ $procesoEjecutandose -gt 0 ]]; then
...
ImprimeHistorialPaginas() {
    ...
    for ((linea = 0; ; linea++)); do
        ...
        if [[ $ultimaPagina -gt ${maxpags[$proceso]} ]]; then
        ...
        #cabeceras de pagina
        for ((m = $primeraPagina - 1; m < $ultimaPagina; m++)); do
            printf "%-${padding}s" " ${paginas[$proceso, $m]}"
        done
        printf "\n"

        for ((i = 1; i <= ${nMarcos[$proceso]}; i++)); do

            printf "%-6s" "M$i"

            for ((j = $primeraPagina - 1; j < $ultimaPagina; j++)); do
                if [[ "${historialApuntador["$proceso", "$i", "$j"]}" == "1" ]]; then
        ...
        if [[ $ultimaPagina -ge ${maxpags[$proceso]} ]]; then
            break
        fi
...
CompruebaFinDeEjecucion() {
    ...
    for estado in "${estadosProcesos[@]}"; do
...
GestionProcesoRoundRobin() {
    ...
    if [[ $conEntradaCPU -eq 1 ]]; then
        if [[ $quantumRestante -le 0 || $procesoEjecutandose -eq 0 || $cambiarProceso -eq 1 ]]; then

            tiempoRestanteEntradaCPU=$(CalculoTiempoTotalEntradaCPU)
            SiguienteProcesoRoundRobin
            if [[ $tiempoRestanteEntradaCPU -ne 0 ]]; then
                hayEntradaCPU=1
            fi
            cambiarProceso=0
        ...
        if [[ $quantumRestante -le 0 || $cambiarProceso -eq 1 || $procesoEjecutandose -eq 0 ]]; then
            SiguienteProcesoRoundRobin
            cambiarProceso=0
        fi
...
EjecutaEntradasCPU() {
    ...
    if [[ $tiempoRestanteEntradaCPU -gt 0 ]]; then
...
SiguienteProcesoRoundRobin() {
    ...
    if [[ $posicionRoundRobin -lt ${#colaRoundRobin[@]} ]]; then
        if [[ $procesoEjecutandose -ne 0 ]]; then
...
CalculaTiempoProcesoEntradaCPU() {
    ...
    if [[ $1 -le 0 ]]; then
        ...
        for ((pagina = $indexPagProc + 2; pagina <= $limsup; pagina++)); do

            if [[ "$paginaAnt" != "$paginaAct" ]]; then
...
GestionFinDeProceso() {
    ...
    if [[ "$procesoEjecutandose" == 0 || $hayEntradaCPU -eq 1 ]]; then
        return 0
    fi

    if [[ $quantumRestante -gt 0 && $(HaFinalizadoProceso $procesoEjecutandose) -eq 0 ]]; then
        return 0
    fi

    if [[ $conEntradaCPU -eq 1 ]]; then

        if [[ $(HaFinalizadoProceso $procesoEjecutandose) -eq 1 ]]; then
        ...
        elif [[ $quantumRestante -le 0 && $antiguoProcesoEjecutandose -ne $procesoEjecutandose ]]; then
        ...
        if [[ $(HaFinalizadoProceso $procesoEjecutandose) -eq 1 ]]; then
            ...
            procesoFinalizado=$procesoEjecutandose
            SacarProcesoMemoria $procesoEjecutandose
        elif [[ $quantumRestante -le 0 ]]; then
            estadosProcesos[$procesoEjecutandose]="En pausa"
            colaRoundRobin+=($procesoEjecutandose)
            # 02-05 COMENTO LA LINEA DONDE MUESTRO POR PANTALLA EL QUANTUM DE TIEMPO
            # logDeEventos+="Se ha ejecutado un quantum de \e[31m$(( $tamQuant - $quantumRestante ))\e[0m del proceso ${nombreProcesoColor[$procesoEjecutandose]}\e[0m\n"
...
ActualizaEstadisticasProcesos() {
    ...
    for ((proceso = 1; proceso <= $nProc; proceso++)); do
        ...
        "En ejecución")
            ...
            let tiempoDeRetorno[$proceso]++
            if [[ $hayEntradaCPU -eq 0 || $conEntradaCPU -eq 0 ]]; then
    ...
    if [[ $tiempoRestanteEntradaCPU -eq 0 ]]; then
        hayEntradaCPU=0
    fi
...
ImprimeEstadoRoundRobin() {
    ...
    if [[ ${#colaRoundRobin[@]} -eq 0 || ${#colaRoundRobin[@]} -lt $posicionRoundRobin || $procesoEjecutandose -eq 0 ]]; then
        echo ""
    else

        for ((i = $posicionRoundRobin - 1; i < ${#colaRoundRobin[@]}; i++)); do
...
DiagramaDeMemoria() {
    ...
    for ((linea = 0; ; linea++)); do
        ...
        for ((m = primerMarco; m <= ultimoMarco; m++)); do
            ...
            if [[ "$procesoActual" != "." ]]; then
                if [[ "$procesoActual" != "$procesoAnterior" ]]; then
        ...
        if [[ $((($numBloquesPorLinea - ($ultimoMarco - $primerMarco)) * $anchoBloque)) -ge $lengthEtiquetaFinal && $colocarEtiqueta -eq 1 ]]; then
            printf "\e[0m|\n"
        else
            printf "\e[0m\n"
        fi
...
DiagramaDeTiempo() {
    ...
    for ((linea = 0; ; linea++)); do
        ...
        for ((m = primerTiempo; m <= ultimoTiempo; m++)); do
            ...
            if [[ $m -eq $tiempoDelSistema && $(CompruebaFinDeEjecucion) -eq 0 && "$procesoActual" != "." ]]; then
                echo -en "${nombreProcesoColor[$procesoActual]}"
                printf "%$((anchoBloque - ${#nombreProceso[$procesoActual]}))s" ""
            else
        ...
        if [[ $((($numBloquesPorLinea - ($ultimoTiempo - $primerTiempo)) * $anchoBloque)) -ge $lengthEtiquetaFinal && $colocarEtiqueta -eq 1 ]]; then
            printf "\e[0m|\n"
        else
            printf "\e[0m\n"
        fi
        ...
        for ((m = primerTiempo; m <= ultimoTiempo; m++)); do
            ...
            if [[ $m -eq $tiempoDelSistema && $(CompruebaFinDeEjecucion) -eq 0 ]]; then
                printf "%${anchoBloque}s" "${VectorTiempoPagina[$m]}"
            else

                if [[ "$procesoActual" != "." ]]; then
                    if [[ $m -gt 0 && "${VectorTiempoPagina[$m]}" == "${VectorTiempoPagina[$(($m - 1))]}" && "$procesoActual" != "$procesoAnterior" ]]; then
        ...
        [ $linea -eq 0 ] && printf "%${anchoEtiqueta}s" "|"
        for ((m = primerTiempo; m <= ultimoTiempo; m++)); do
            if [[ "${VectorTiempoPagina[$m]}" != '<' && "${VectorTiempoPagina[$m]}" != '>' ]]; then
                procesoActual=${VectorTiempoProceso[$m]}
            fi

            if [[ "$procesoActual" != "$procesoAnterior" && ("${VectorTiempoPagina[$m]}" != '<' && "${VectorTiempoPagina[$m]}" != '>') ]]; then
